    ...
    if (flag) {
      ...
      try {
        ...
        if (userInfo.username === 'sysAdmin' || userInfo.username === 'admin' || userInfo.isSystem) {
          console.log(`✅ 超级管理员访问放行: ${userInfo.username}`);
          await next();
          return;
        }
        ...
        if (userInfo.userRoleIds && userInfo.userRoleIds.length > 0) {
          ...
          if (hasSuperAdminRole) {
            console.log(`✅ 超级管理员角色访问放行: ${userInfo.username}`);
            await next();
            return;
          }
        }
        ...
        // 2. 如果没有直接匹配，检查按钮权限
        if (!hasPermission) {
          const buttonMenus = userMenus.filter(menu =>
            (menu.menuType === 'button' || menu.menuType === 'btn') &&
            menu.btnPower
          );
